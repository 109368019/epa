...
for num, video_name in enumerate(video_list):
    ...
    video_path = './綠電楊梅_鏡頭17~20/0625/video_red/video_{}'.format(directory_name)
    ...
    while success:
        ...
        if count > frame_count - 1:  # 若是超出影片frame數，則跳出
            break
        ...
        if count > para.start_frame or is_first_statuation:  # 開始
            ...
            if count < para.start_frame + para.window_length and not is_first_statuation:
            ...
            if count > para.start_frame + para.window_length or is_first_statuation:
                ...
                # 0625 綠環境A10===========================================
                area1 = np.array([[0,0],[140,0],[510,193],[620,0],[720,0],[720,480],[0,480]])
                ...
                cv2.polylines(frame, area_list, True, (0, 255, 255))
    ...
    json_Name = "./綠電楊梅_鏡頭17~20/0625/log/" + directory_name + ".json"
